...
1. Добавлены операторские кнопки `Копировать JSON` и `Очистить лог`.
2. Реализован copy-flow для текущих табличных данных (latest rows -> clipboard).
3. Лог теперь можно быстро сбрасывать без перезапуска скрипта.
